...
class BlogServiceTest {
  ...
  private static HttpRequest createBlogPostRequest(Map<String, String> content)
    ...
    return HttpRequest.builder().post("/blogs")
        .content(MediaType.JSON_UTF_8, mapper.writeValueAsString(content)).build();
  }
  ...
  void createBlogPost() throws JsonProcessingException {
    ...
    final HttpRequest request = createBlogPostRequest(
        Map.of("title", "My first blog", "content", "Hello Armeria!"));
    final AggregatedHttpResponse res = client.execute(request).aggregate().join();

    final Map<String, Object> expected = Map.of("id", 0, "title", "My first blog", "content",
        "Hello Armeria!");
  ...
  void getBlogPosts() throws JsonProcessingException {
    final WebClient client = WebClient.of(server.httpUri());
    final HttpRequest request = createBlogPostRequest(
        Map.of("title", "My second blog", "content", "Armeria is awesome!"));
    ...
    final List<Map<String, Object>> expected = List.of(
        Map.of("id", 1, "title", "My second blog", "content", "Armeria is awesome!"),
        Map.of("id", 0, "title", "My first blog", "content", "Hello Armeria!"));
    assertThatJson(res.contentUtf8()).whenIgnoringPaths("[*].createdAt", "[*].modifiedAt")
        .isEqualTo(mapper.writeValueAsString(expected));
  }
  ...
  @Test
  @Order(4)
  void updateBlogPost() throws JsonProcessingException {
    final WebClient client = WebClient.of(server.httpUri());
    final Map<String, Object> updatedContent = Map.of("id", 0, "title", "My first blog", "content",
        "Hello awesome Armeria!");

    final HttpRequest updateBlogPostRequest = HttpRequest.builder().put("/blogs/0")
        .content(MediaType.JSON_UTF_8, mapper.writeValueAsString(updatedContent)).build();
  ...
  void badRequestExceptionHandlerWhenTryingDeleteMissingBlogPost() throws JsonProcessingException {
    ...
    assertThatJson(res.contentUtf8()).isEqualTo(
        "{\"error\":\"The blog post does not exist. ID: 100\"}");
